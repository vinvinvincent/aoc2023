...
class Workflow:
    ...
    expressions_eval_str: str = None


@dataclass
class Parts:
    xmas: dict[int] = field(default_factory=lambda: {})


@dataclass
class Receipt:
    parts_list: List[Parts] = field(default_factory=list)
    workflows: dict[Workflow] = field(default_factory=lambda: {})
...
def parse_workflow(line):
    ...
    expressions_str = tokens[1]

    for expression_str in expressions_str.split(','):
        expression = Expression()
        items = expression_str.split(':')

        if len(items) == 1:
            # end node
            expression.destination = items[0]
        else:
            # condition node
            expression.conditon = items[0]
            expression.destination = items[1]

        workflow.expressions.append(expression)

    # shortcut for eval()
    expressions = workflow.expressions
    workflow.expressions_eval_str = ''

    match len(workflow.expressions):
        case 1:
            workflow.expressions_eval_str = f'"{expressions[0].destination}"'
        case 2:
            workflow.expressions_eval_str = (
                f'"{expressions[0].destination}" if {expressions[0].conditon} '
                f'else "{expressions[1].destination}"'
            )

        case 3:
            workflow.expressions_eval_str = (
                f'"{expressions[0].destination}" if {expressions[0].conditon} '
                f'else ("{expressions[1].destination}" if {expressions[1].conditon} '
                f'      else "{expressions[2].destination}")'
            )
        case 4:
            workflow.expressions_eval_str = (
                f'"{expressions[0].destination}" if {expressions[0].conditon} '
                f'else ("{expressions[1].destination}" if {expressions[1].conditon} '
                f'      else ("{expressions[2].destination}" if {expressions[2].conditon} '
                f'            else "{expressions[3].destination}"))'
            )
        case _:
            raise NotImplemented('Unhandled Num of Rules')
...
def parse_parts(line):
    ...
    parts = Parts()
    parts.xmas = {'x': int(x), 'm': int(m), 'a': int(a), 's': int(s)}
    return parts
...
def parse_receipt(text):
    ...
    for line in text.split('\n'):
        ...
        if not found_empty_line:
            ...
            receipt.workflows[workflow.name] = workflow
        else:
            receipt.parts_list.append(parse_parts(line))
...
SAMPLE_DATA_TEXT = '''px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
...
PUZZLE_DATA_TEXT = '''btl{m>2179:hzd,a>965:js,x<3167:bc,crx}
...
SAMPLE_DATA = parse_receipt(SAMPLE_DATA_TEXT)
PUZZLE_DATA = parse_receipt(PUZZLE_DATA_TEXT)
