...
class Games:
    ...
    def parse_game_string(self, game_string):
        game_list = game_string.split('\n')
        for game in game_list:
            game_info = game.split(':')[1]
            sets = self.parse_cubes(game_info)
            self._games.append(sets)

    def parse_cubes(self, game_info):
        cubes_list = []
        cubes_infos = game_info.split(';')
        for cubes_info in cubes_infos:
            cubes_info = cubes_info.strip()
            if cubes_info:
                cube_counts = self.parse_cube_counts(cubes_info)
                cubes = Cubes(
                    cube_counts['red'], cube_counts['green'], cube_counts['blue']
                )
                cubes_list.append(cubes)
        return cubes_list

    def parse_cube_counts(self, cubes_info):
        cube_counts = {'red': 0, 'green': 0, 'blue': 0}
        tokens = cubes_info.split(',')
        for token in tokens:
            token = token.strip()
    ...
    def get_game_number(self, game_number):
        return self._games[game_number - 1]

    def total_games(self):
        return len(self._games)


SAMPLE_DATA_TEXT = '''Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green'''

PUZZLE_DATA_TEXT = '''Game 1: 9 red, 5 blue, 6 green; 6 red, 13 blue; 2 blue, 7 green, 5 red
...
SAMPLE_DATA = Games(SAMPLE_DATA_TEXT)
PUZZLE_DATA = Games(PUZZLE_DATA_TEXT)
